...
const getTimeRange = (range: string, dateStr?: string) => {
...
router.get('/compare', async (req, res, next) => {
    try {
        ...
        const { startDate, endDate } = getTimeRange(time_range as string, date as string);
        ...
        for (const site of sites) {
            ...
            const productionQuery = await prisma.$queryRaw<any[]>`
            ...
            const downtimeQuery = await prisma.$queryRaw<any[]>`
