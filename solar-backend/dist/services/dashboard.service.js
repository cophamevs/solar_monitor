...
class DashboardService {
    ...
    static async getSummary() {
        ...
        const latestPower = await index_js_1.prisma.telemetry.findMany({
            where: {
        ...
        const latestEnergy = await index_js_1.prisma.telemetry.findMany({
            where: {
        ...
        const latestTotalEnergy = await index_js_1.prisma.telemetry.findMany({
            where: {
        ...
        const deviceStats = await index_js_1.prisma.device.groupBy({
            by: ['status'],
            _count: true
        });
        // Count open alerts
        const openAlerts = await index_js_1.prisma.alert.count({
            where: { status: 'OPEN' }
        });
    ...
    static async getAlarmSummary() {
        const alertCounts = await index_js_1.prisma.alert.groupBy({
            by: ['level'],
            where: { status: 'OPEN' },
            _count: true
        });
        ...
        for (const count of alertCounts) {
            const level = count.level.toLowerCase();
     ...
     */
    static async getEnergyFlow(siteId) {
        ...
        const latestReadings = await index_js_1.prisma.telemetry.findMany({
            where: {
